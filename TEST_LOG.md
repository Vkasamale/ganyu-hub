...
Last updated: 2026-08-04 (Full E2E test walk — job activity timeline sessions 1-4 largely confirmed live; see BUG-001 line and BUG-007 for the one new bug found)
...
⚠️ **Job activity timeline (session 4) — revision limits + paid overage** — proposals carry `revisions_offered` and optional `extra_revision_rate`; on accept both copy to `jobs.revisions_included` / `jobs.extra_revision_rate` — confirmed live (Job C1: `revisions_included=1`, `extra_revision_rate=5000` after accept). Within-limit revision is free and logs `revision_requested`, counter 0→1 — confirmed live. Second-job blank-rate path shows the "didn't offer paid extras" message and does not increment — confirmed live (Job C2). **Paid overage top-up is broken**: clicking "Pay MWK 5,000 & continue" never creates a `payment_topups` row and the counter never advances past the included count — see **BUG-007** (RLS blocks the client-initiated insert). X-of-Y indicator not separately re-verified beyond the DB counter check.
