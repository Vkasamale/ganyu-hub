...
🕒 **Job activity timeline (session 4) — revision limits + paid overage** — proposals now carry `revisions_offered` and optional `extra_revision_rate`; on accept, both copy to `jobs.revisions_included` / `jobs.extra_revision_rate`, and `jobs.revisions_used` tracks against the limit. Client's "Request changes" action counts revisions: within the included limit it's free and logs `revision_requested`; over the limit with `extra_revision_rate` set it triggers the existing top-up escrow rail (`requestTopUp`/`payTopUp`) for the extra revision amount and only logs `revision_requested` + increments the counter after payment clears (webhook-driven); with `extra_revision_rate = null` it shows a "not available" message and neither logs nor increments. Timeline component surfaces a "Revisions: X of Y used" indicator at the top. Verify manually per session-4 browser steps: (1) submit proposal with revisions=1 / rate=MWK 5,000, accept, confirm job fields populated in Studio; (2) first revision request is free, counter 0→1, event lands; (3) second request shows overage prompt, sandbox top-up payment completes, counter 1→2, event lands only after payment; (4) second test job with blank rate shows "not available" and neither logs nor increments; (5) X-of-Y indicator updates after each request. Migration required for all four new columns.

🕒 **Job activity timeline (session 3) — file delivery** — new `job-deliverables` Supabase Storage bucket (private per-job, path `<job_id>/<uuid>.<ext>`) with RLS scoped so the accepted creative writes and both parties + admin read. New `JobDeliverySubmit` client component on the job page (creative-only, active job only): file picker with 10MB cap or external-link textbox (Google Drive / WeTransfer / Dropbox), mutual exclusion enforced client + server, optional short note. Server action `submitDelivery` uploads via service role, saves `{ file_url, file_name, file_type }` or `{ external_link }` in `job_events.metadata`, and detects whether the most recent relevant event was `revision_requested` — if so, logs `revision_delivered` instead of `files_delivered` so the timeline reads correctly. Timeline extended to render download links or external links inline. Verify manually per session-3 browser steps: (a) creative uploads under-10MB file with note → downloadable timeline row; (b) over-10MB rejected inline with no upload attempt; (c) external link path → timeline row labeled "External link"; (d) Supabase Studio row shapes; (e) client can see both; (f) unrelated third account cannot fetch the bucket file directly. Revision detection cannot be end-to-end tested yet — no revision_requested trigger until session 4.

🕒 **Job activity timeline (session 2) — status transitions wired** — `logJobEvent` now called from every lifecycle mutation: escrow_funded (PayChangu callback + webhook, atomic guard so only one wins), work_started (both `promotePendingAcceptance` for payment-first accept and `confirmScope` when both parties confirm), job_completed + cancelled (`updateJobStatus`), dispute_filed (`raiseDispute` + `cron/non-response-check` for the 72h auto-flag with null actor + "Auto-flagged" note), dispute_resolved + cancelled (`adminResolveDispute`, `adminResolveCancellation`), deadline_extended (`respondToDeadlineExtension` on approve). Full paid-flow sandbox walk needed: post → propose → accept → fund → mark complete → release payment, confirm every stage lands on the timeline in order. Second test job: raise a dispute, confirm `dispute_filed` row with client/creative as actor. Supabase Studio: verify actor_id is populated correctly on each row (client for accept/complete, admin for resolve, null for cron auto-flag).
