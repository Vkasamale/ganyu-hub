...
- **[BUG-017] The `payment_released` event is silently dropped — no record of when a creative was paid.** — found 2026-08-07 by a real sandbox release, payments/observability. **Fixed in repo; ALTER run on the live DB 2026-08-07. Not yet re-tested with a real release.**
  ...
  - Fix: `'payment_released'` added to the constraint in `schema.sql`. **`create table if not exists` will not alter an existing table — the live DB needs the explicit `alter table … drop constraint / add constraint` run by hand.**
