...
| ✅ | L5 footer | Shipped — `components/footer.tsx`, `/release-notes` |
| ✅ | L6 closing CTA | Shipped |
| ✅ | L7 proof row | Pre-existing, live on real numbers (`app/page.tsx:30`) |
| ✅ | L8 testimonials | Shipped — `components/home-proof.tsx`, reads `reviews` (≥4★, comment ≥60 chars). Hidden below 3. |
| ✅ | L9 featured creatives | Shipped — profiles with ≥1 portfolio item, hidden below 6. Live. |
| ✅ | L10 success story | Shipped as a stated "not yet" — fill the `SUCCESS_STORY` constant to turn it on. |
| ✅ | L11 trusted-by | Shipped as a stated "coming soon" — add ≥4 names to `TRUSTED_BY`, with written permission. |

Landing page is 11 of 11. **Phases 0–4 complete** (Phase 3 and 4 form paths unverified) (items 1–25; 16 and 17
...
Deferred deliberately: accessibility statement (§J3 #7) — founder's copy to
write, not a developer's to invent.
...
- **Also shipped (2026-08-07):** extra-revision top-ups no longer appear in the client's "Accept & pay" panel (BUG-010); creative→client review links now route to `/clients/…` (BUG-011); a `payment_released` job event now records **when** a creative is paid — forward-only, nothing can reconstruct past releases.
...
| Escrow release observability: prove `payment_released` is written, once | DONE 2026-08-07 (BUG-018 fix unverified) | BUG-017 verified live on job `d2a9aea7…` — event present with a real timestamp, so the `/clients/[id]` release-speed figure now has data. Same query exposed BUG-018 (event written twice, `reconcile` racing `webhook`); fixed by compare-and-swap on both writers, still needs one live release showing exactly one row. Root cause addressed too: `JobEventType` derives from a runtime `JOB_EVENT_TYPES` array and `tests/lib/job-event-types.test.ts` fails if it drifts from the SQL constraint. |
| Job page money-state clarity: make held / released / disputed visually distinct, surface payment above the fold, collapse the long cards | BUILT 2026-08-07, UNVERIFIED | Coloured stamp badge in the title card (keyed map — one added key covers partial deposits). Payment card moved under the header. Brief, activity and delivery collapse via native `<details>` (`components/collapsible.tsx`). Dispute demoted from standing card to a button in the action row. Nothing looked at in a browser yet — see TEST_LOG. |
